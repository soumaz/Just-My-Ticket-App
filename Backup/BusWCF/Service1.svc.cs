...
namespace BusWCF
{
    ...
    public class Service1 : IService1
    {
        ...
        static string[] buses;
        static string transaction_result;
        ...
        public string Current_Balance(string email)
        {
            var balance = (from temp in busObject.REGISTERs where temp.Email_ID==email select temp.Bus_Money).SingleOrDefault<System.Int32?>();
        ...
        {
            busObject.usp_UpdateDetailsTransaction(money, email, ref transaction_result);
            busObject.SubmitChanges();
        ...
        public List<Bus> BusRoute(string source, string destination)
        {
            ...
            {
                Bus temp = new Bus {Bus_No=item.Bus_No,Distance=item.Distance,Source=item.Source,Destination=item.Destination,Mappath=item.Map_Path,Duration=item.Duration,Rush=item.Rush};
                resultBus.Add(temp);
            }
            return resultBus;
        }
